...
### Q1: What happens to tokens that are no longer created by any card?

If a card is edited to remove a token-creating ability, and that was the only card creating that token, should the token be automatically removed from the deck?

**Proposed answer**: No automatic removal. Stage the change as "token source_cards became empty" and let the user decide whether to delete it in the Assembly Line. The Tokens tab should visually flag tokens with no `source_cards` as "orphaned" so the user knows to clean them up.

### Q2: Should we support DFC tokens (tokens that transform)?

For now: No. DFC tokens are rare and complex. The token editor will only support single-faced tokens. Add a note to revisit.

### Q3: Multiple artworks for a token
...
In the UI, the Tokens tab should show all artwork variants for a token (stacked thumbnails or a carousel). A "+" button adds a new artwork variant by forging a new image to `Staging/<TokenName>_N.jpg`.

For now (Phase 1 of token rework), leave multiple artworks as a future task. Just make the existing logic work correctly.

### Q4: Token discovery for back faces of DFCs and subspells
...
This should be handled in `card.get_tokens()` in `src/core/card.py` (or wherever `card.get_tokens` is implemented — confirm this exists). If it only checks `card.rules`, it needs updating.

### Q5: Common tokens vs. specialized tokens in the new format
...
For the Cockatrice fix, we need to handle common tokens properly. Currently in the deck JSON they appear with `complete: 1` but may have no image in `Tokens/`. For common tokens with no image, the Cockatrice exporter prints a warning. This is acceptable behavior — common tokens use Cockatrice's built-in token database.

**Open question for user**: Should common tokens that have no local image file be included in the Cockatrice `.cod` predefined tokens zone? If yes, they'll appear in the token list but have no artwork (Cockatrice will use its built-in image or a blank). If no, we skip them. My recommendation: include them, since having them in the predefined zone is still useful (quick access to the token in-game), even without custom art.
...
Common tokens (Clue, Treasure, Food, etc.) from `common_tokens.json` often have no local image file in `Tokens/`. Should they still be listed in the `.cod` predefined tokens zone (they'll show in Cockatrice's token list but use its built-in art or a blank), or should they be skipped unless a local image exists?

### Needed for Phase T4 (Forge-Time Auto-Discovery)

**Item 5 — Token discovery scope**  
Should token discovery scan only `front.rules`, or also `back.rules` (DFC back face) and `subspell.rules` (Adventure/Omen subspell)? My recommendation is all three, but confirm.

### Deferred / Lower Priority

- **Multiple artwork variants UI**: In scope for this rework, or deferred to a later phase?
- **Renamed card + source_cards**: If a card is renamed, should `source_cards` on its tokens auto-update? (Recommendation: yes, handle in forge endpoint.)
- **Token deletion from Assembly Line**: If auto-discovery removes a token's last `source_card`, should the token be staged for deletion automatically, or just flagged as orphaned?
