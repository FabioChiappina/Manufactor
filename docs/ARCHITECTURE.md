...
**Token Keys:**
- `_TOKEN_*`: Token definitions (use this prefix for token keys in the `tokens` section)

**See Also:**
- [JSON_FORMAT.md](JSON_FORMAT.md) - Complete JSON format reference
- [DOUBLE_FACED_CARDS.md](DOUBLE_FACED_CARDS.md) - Double-faced cards documentation
- [SUPERTYPES.md](SUPERTYPES.md) - Supertype fields documentation
...
- `metadata` section with deck information (name, description, commander, format, etc.)
- `cards` section with all card definitions using `front`/`back` structure
- `tokens` section with token definitions and source tracking
- `quantity` field for specifying number of copies
- `source_cards` field to track which cards create tokens
- `setname` field in metadata as deck-wide default (cards can override individually)
- Support for single and multiple commanders (partner commanders, etc.)
- Double-faced cards use "Front Name / Back Name" key format
...
1. Auto-detects format based on presence of `metadata` key
2. Loads metadata section (deck info, commander, format, etc.)
3. Creates Card objects from `cards` section (with `front`/`back` structure)
4. Loads tokens from `tokens` section
5. Handles missing fields with defaults (None values)
6. Processes tags and aggregates them by supertag
